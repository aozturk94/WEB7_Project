        ...
        public List<Route> GetRoute(string startLocation, string endLocation)
        {
            using (var context = new BookingContext())
            {
                ...
                var routes = context.Routes
                    .FromSqlRaw($"Select * From Routes Where ((StartLocation='{start[0]}' or FirstStation='{start[0]}' or SecondStation='{start[0]}' or ThirdStation='{start[0]}' ) and (EndLocation='{end[0]}' or ThirdStation='{end[0]}' or SecondStation='{end[0]}' or FirstStation='{end[0]}' )) ")
                    .ToList();
